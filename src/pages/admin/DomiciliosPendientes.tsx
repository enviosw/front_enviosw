...
const estadoConfig: Record<number, { label: string; bg: string; text: string; dot: string; ring: string; border: string; pill: string }> = {
  1: { // PENDIENTE -> VERDE (Bootstrap)
    label: "PENDIENTE",
    bg: "bg-success",
    text: "text-white",
    dot: "bg-white",
...
const Field: React.FC<{ label: string; children: React.ReactNode; icon?: React.ReactNode }> = ({ label, children, icon }) => (
...
const PhoneIcon = () => (
  <svg width="16" height="16" viewBox="0 0 24 24" fill="none" aria-hidden>
    <path d="M2 5c0-1.1.9-2 2-2h2c.8 0 1.5.5 1.8 1.2l1.1 2.6c.3.7.1 1.6-.5 2.1l-1.2 1c1.1 2.3 3 4.2 5.3 5.3l1-1.2c.5-.6 1.4-.8 2.1-.5l2.6 1.1c.7.3 1.2 1 1.2 1.8v2c0 1.1-.9 2-2 2h-1C9.6 22 2 14.4 2 5V5Z" stroke="currentColor" strokeWidth="1.5"/>
  </svg>
);
const OriginIcon = () => (
...
const DestIcon = () => (
  <svg width="14" height="14" viewBox="0 0 24 24" fill="none" aria-hidden>
    <path d="M12 22V2M19 15l-7 7-7-7" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"/>
  </svg>
);

/** Util: fecha + hora (CO) */
const formatFechaHora = (iso?: string | null, hora?: string | null) => {
  const base = iso ? (formatDate?.(iso) ?? new Date(iso).toLocaleDateString("es-CO", { timeZone: "America/Bogota" })) : "—";
...
const DomicilioItem: React.FC<{ d: Domicilio }> = ({ d }) => {
    ...
    >
      <div className={cx("rounded-xl bg-white/80 p-3 backdrop-blur", "ring-1 ring-inset", cfg.ring)}>
        ...
        <div className="mt-4 grid grid-cols-1 gap-3 sm:grid-cols-2">
          <Field label="Origen" icon={<OriginIcon />}>{d.origen_direccion || "—"}</Field>
          <Field label="Destino" icon={<DestIcon />}>{d.destino_direccion || "—"}</Field>
        </div>
        ...
        <div className="rounded-2xl bg-gradient-to-r from-indigo-600 via-fuchsia-600 to-rose-600 p-[1px] shadow-sm">
          <div className="flex flex-wrap items-center justify-between gap-3 rounded-2xl bg-white/95 px-3 py-2">
            ...
            <div className="flex items-center gap-2">
              ...
              >
                <svg width="16" height="16" viewBox="0 0 24 24" fill="none" aria-hidden>
                  <path d="M4 4v6h6M20 20v-6h-6M6.5 6.5a7.5 7.5 0 0 1 12.3 2.7M17.5 17.5A7.5 7.5 0 0 1 5.2 14.8" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round"/>
                </svg>
