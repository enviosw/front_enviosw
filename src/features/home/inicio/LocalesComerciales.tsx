...
import { FaMapMarkerAlt, FaSearch, FaTimes, FaWhatsapp, FaRedo, FaChevronDown } from 'react-icons/fa';
...
const LocalesComerciales: React.FC<{ servicioId: number | null }> = ({ servicioId }) => {
  ...
  const { data, isLoading, isError, refetch } = useComerciosPublicos({ servicioId, search, page });

  const lastPage = data?.lastPage ?? 1;
  const hasMore  = page < lastPage;

  // Resetear al cambiar servicio o búsqueda
  useEffect(() => {
    setPage(1);
    setLocales([]);
  }, [servicioId, search]);
  ...
  useEffect(() => {
    if (searchValue === '') { setSearch(''); return; }
    const t = setTimeout(() => setSearch(searchValue.trim()), 300);
    return () => clearTimeout(t);
  }, [searchValue]);

  const handleClearSearch = () => { setSearchValue(''); setSearch(''); };

  const handleCargarMas = () => setPage((p) => p + 1);

  // WhatsApp
  const limpiarTelefono = (t?: string | null) => t ? String(t).replace(/\D/g, '') : '';
      ...
      <div className="flex justify-center items-center mb-5">
        <div className="relative w-full max-w-full lg:max-w-md flex items-center">
          <input
            ...
            onKeyDown={(e) => e.key === 'Enter' && setSearch(searchValue.trim())}
            placeholder="Buscar negocio o servicio…"
            className="w-full py-3 pl-5 pr-12 bg-white border-2 border-[#EDE8E3] rounded-full text-[#1A1208] placeholder-[#6B5E52]/60 font-medium text-sm focus:outline-none focus:ring-2 focus:ring-[#E8622A]/40 focus:border-[#E8622A] shadow-sm hover:shadow-md transition-all duration-300"
          />
          {searchValue ? (
            ...
            <button onClick={() => setSearch(searchValue.trim())} aria-label="Buscar"
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 2xl:grid-cols-5 gap-3 lg:gap-5">
        {locales.map((comercio, idx) => (
          <div
            ...
            className="group cursor-pointer bg-white border border-[#EDE8E3] rounded-2xl overflow-hidden relative shadow-[0_2px_12px_rgba(30,20,10,0.07)] hover:shadow-[0_8px_28px_rgba(30,20,10,0.15)] hover:scale-[1.02] hover:-translate-y-1 transition-all duration-300 ease-in-out"
          >
      ...
      <div className="flex flex-col items-center py-8 gap-4">
        ...
        {!isLoading && !isError && hasMore && (
          <button
            ...
            className="flex items-center gap-2 bg-white border-2 border-[#E8622A] text-[#E8622A] hover:bg-[#E8622A] hover:text-white font-semibold px-6 py-2.5 rounded-full transition-all duration-300 hover:scale-105 shadow-sm"
          >
